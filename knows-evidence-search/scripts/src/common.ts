...
export const SOURCES: Record<EvidenceSource, SourceConfig> = {
  paper_en: {
...
export function isEvidenceSource(value: string | undefined): value is EvidenceSource {
  return Boolean(value && value in SOURCES);
}

export function baseUrlEnv(name: "KNOWS_BASE_URL"): string {
  return process.env[name] || DEFAULT_BASE_URL;
}

export function optionalEnv(name: "KNOWS_API_KEY"): string | undefined {
...
export async function postJson(baseUrl: string, endpoint: string, apiKey: string | undefined, payload: unknown): Promise<unknown> {
